...
static inline void ip_to_bytes(__u32 ip_addr, __u8 *byte1, __u8 *byte2,
...
int fastbroadcast(struct __sk_buff *skb) {
  ...
  if (curr < tgt_list->max_count) {
    ...
    // bpf_printk("[fastbroad_prog] clone packet, payload size: %d, max_count:
...
int xdp_sock_prog(struct xdp_md *ctx) {
  ...
  if (bpf_map_lookup_elem(&qidconf_map, &index)) {
    ...
    // 		{
    // #ifdef DEBUG_XDP
    // 			bpf_printk("[fastdrop_prog] bpf_redirect_map to xsk
    // map.\n"); #endif 			return bpf_redirect_map(&xsks_map, index, 0);
    // 		}
    ...
    // 			}
    // 			// bpf_printk("[fastdrop_prog]i: %d cursor: %c\n", i,
    // *(cursor+i)); 			update_time = update_time * 10 + (*(cursor + i) - '0');
    // 		}
    // 		/* Lookup current metadata*/
    // 		__u32 key = 0;
    // 		struct metadata *current_metadata = bpf_map_lookup_elem(&metadata_map,
    // &key); 		if (!current_metadata)
    // 		{
    // 			return bpf_redirect_map(&xsks_map, index, 0); // No metadata found,
    // handle in userspace
    // 		}

    // 		if (update_time == current_metadata->update_time)
    // 		{
    // 			goto drop; // Drop packet if update time is same as current
    // metadata
    // 		}
    ...
    // 			bpf_map_update_elem(&metadata_map, &key, &new_metadata,
    // BPF_ANY); 			goto drop;
    // 		}
